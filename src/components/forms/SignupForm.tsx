...
export default function SignupForm() {
  ...
  const onSubmit = handleSubmit(async () => {
    ...
    const confirmation = `PBW-${Math.random().toString(36).slice(2, 7).toUpperCase()}`
    setSubmissionId(confirmation)
    reset(defaultValues)
  })
      ...
      {submissionId && (
        <div className="mt-8 rounded-2xl border border-green-200 bg-green-50 p-6 text-sm text-green-700">
          ...
          <p className="mt-2">
            ...
            <span className="font-semibold">{submissionId}</span>. An agent will call within one business day to finalize
            provider access and payment details.
          </p>
        </div>
      )}
    </div>
