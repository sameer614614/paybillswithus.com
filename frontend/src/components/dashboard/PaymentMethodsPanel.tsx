...
function PaymentMethodsPanel({ profile }: { profile: Profile | null }) {
  ...
  const createMutation = useMutation({
    mutationFn: (values: PaymentMethodFormValues) => {
      ...
      const expMonthValue = values.type === 'BANK_ACCOUNT' ? null : Number(values.expMonth)
      const expYearValue = values.type === 'BANK_ACCOUNT' ? null : Number(values.expYear)
      ...
      const payload = {
        ...
        securityCode:
          values.type === 'BANK_ACCOUNT' ? undefined : values.securityCode ? values.securityCode.trim() : undefined,
        billingAddress,
  ...
  const handleEditSubmit = () => {
    ...
    const updates: Partial<CreatePaymentMethodInput> & { isDefault?: boolean } = {
      ...
      expMonth: editing.type === 'BANK_ACCOUNT' || !editing.expMonth ? null : Number(editing.expMonth),
      expYear: editing.type === 'BANK_ACCOUNT' || !editing.expYear ? null : Number(editing.expYear),
